...
`f%d=f-[f/d]*d`，`[f/d]`表示不大于`f/d`的最大可能整数。当`f`和`d`中至少有一个是负浮点数，按正浮点数计算，结果符号同f符号。
`浮点数%0`的值为`Double.NaN`。
`-0%3`值为`0`（因为在补码中`+0`和`-0`是一样的），`-0.0%3`值为`-0.0`。

### 自增、自减

自增、自减运算符，分为前缀式（先进行加1运算，再使用变量的值）与后缀式（先使用变量的值，再进行加1运算）。
...
除了~，>>，<<和>>>外，其它位运算符都可以用于处理布尔操作数（Java将true和false作为单比特来对待，而不管它在具体实现中的长度）。

被操作数>>位数：被操作数/2^位数%模^

被操作数<<位数：被操作数*2^位数%模^

被操作数为int时，模为32；被操作数为long时，模为64。因此，x>>>32（x为int类型）的值不会变，因为32的模为0。被操作数为short、byte或char类型时，先自动转换成int类型，再进行移位运算，结果也是int类型。
...
1. 在声明语句初始化时，右边的数值不自动转换为`int`类型，而是跟左边的类型相同，但赋值时，右边的值会进行整型提升。要注意，`short s = 1; s = s + 1;` 是错误的，而`short s = 1; s += 1;` 则是允许的。这只是针对整型而言，其它情况，则必需强制转换。如：`int i = 0.123`是不合法的；
